...
class _OnBoardState extends State<OnBoard> {
              ...
              builder: (context, doctorSnapshot) {
                if (doctorSnapshot.hasData) {
                  if (doctorSnapshot.data!.exists) {
                    ...
                    return profileCheck(doctorSnapshot, FormValidInfo.doctorInfo, PublicationsScreen());
                  } else {
                    // Kullanıcı bir hasta olabilir. Hasta koleksiyonundan kontrol edelim.
                    return StreamBuilder<DocumentSnapshot>(
                      stream: _firestore.collection('patients').doc(user.uid).snapshots(),
                      builder: (context, patientSnapshot) {
                        if (patientSnapshot.hasData) {
                          if (patientSnapshot.data!.exists) {
                            // Kullanıcı bir hasta.
                            return profileCheck(patientSnapshot, FormValidInfo.patientInfo, PublicationsScreen());
                          } else {
                            ...
                            ]),body:  Center(child: CircularProgressIndicator()));; // Bu bir örnek. Böyle bir ekranınızın olup olmadığını kontrol edin.
                          }
                        }
  ...
  Widget profileCheck(AsyncSnapshot<DocumentSnapshot> snapshot, FormValidInfo info, Widget mainPage) {
    if (snapshot.data!.get('completedProfile') == true) {
      return mainPage; // Profili tamamlanmış.
    } else {
      return InformationRecord(formValidInfo: info); // Profil bilgilerini tamamla sayfasına yönlendir.
    }
  }
}
